...
class Person {
  ...
  public:
    Person(string _name, int _groupID) : name(_name), groupID(_groupID) {}
    Person(const Person &copy) : Person(copy.name, copy.groupID) {}
    virtual ~Person() {}

    virtual int GetType(void) { return ID::PERSON; }

    string GetName(void) { return name; }
    int GetGroupID(void) { return groupID; }
    virtual void Show(string groupTitle) { return; }
...
class Group {
  ...
  public:
    Group(int _id, string _title) : id(_id) { title = _title; }
    Group(const Group &copy) : Group(copy.id, copy.title) {}
    int GetID(void) { return id; }
    string GetTitle(void) { return title; }
    void SetId(int _id) { id = _id; }
    void SetTitle(string _title) { title = _title; }
    void Show(void) { cout << GetID() << "            " << GetTitle() << endl; }
};
...
class GroupList {
  ...
  public:
    GroupList(void) { index = 0; }
    ~GroupList(void) {
        for (int i = 0; i < index; i++) {
            delete labList[i];
        }
    }
    int GetIndexFromOne(void) { return index + 1; }
...
class Student : public Person {
  ...
  public:
    ...
    int GetType(void) { return ID::STUDENT; }
...
class Mentor : public Person {
  ...
  public:
    ...
    int GetType(void) { return ID::MENTOR; }

    void Show(string groupTitle) {
        cout << name << "    " << groupID << "   " << groupTitle << "   " << id
             << "     " << company << endl;
    }

    int GetID(void) { return id; }
    string GetCompany(void) { return company; }
};
    ...
    static int Main(void) {
        ...
        while (1) {
            ...
            if (command == 0) {
                ...
                if (group == NULL) {
                    cout << "ID에 맞는 Group을 찾지 못함" << endl << endl;
                    continue;
                }
